...
> the parenthesis used for casting is called **_casting operator_**

# String Concatenation Operator

1. Opération are evaluated left to right.
...
If the right-hand operand is infinity, the resul will be same as left operand:
...
unary operators promote byte, short or char to an `int`;
...
The compound operator `+=` is applicable to `String`
...
The logical NOT operator (`!`) is a unary operator tat negates the boolean value of its operand

# Logical Short-Circuit AND operator (`&&`)

- The `&&` operator returns `true` **only** if both operands are `true`
- It's referred as a _"short-circuit"_ operator because, if the left-hand operand evaluates to `false`,
...
| 10 | left-right | `<`, `<=`, `>`, `>=`<br>`instanceof`| number<br>reference, type | less than, greater than<br>type comparison |
| 9 | left-right |`==`, `!=` | primitive, reference | equality comparison |
...
| 5 | left-right| `&&` | boolean| conditional AND |
| 4 | left-right| <code>&#124;&#124;</code> | boolean | conditional OR |
| 3 | right-left | `?:`| | |
